...
test_remote_pins() {
  ...
  test_expect_success "'ipfs pin remote add'" '
    ...
    export ID_B=$(ipfs pin remote add --enc=json $BASE_ARGS --name=name_b $HASH_B | jq --raw-output .RequestID) &&
    export ID_C=$(ipfs pin remote add --enc=json $BASE_ARGS --name=name_c $HASH_C | jq --raw-output .RequestID) &&
  ...
  test_expect_success "'ipfs pin remote ls' for existing pins by ID" '
    FOUND_ID_A=$(ipfs pin remote ls --enc=json --cid=$HASH_A | jq --raw-output .RequestID | grep $ID_A) &&
  ...
  test_expect_success "'ipfs pin remote ls' for existing pins by name" '
    FOUND_ID_A=$(ipfs pin remote ls --enc=json --name=name_a | jq --raw-output .RequestID | grep $ID_A) &&
    echo ID_A=$ID_A FOUND_ID_A=$FOUND_ID_A &&
    echo $ID_A > expected &&
    echo $FOUND_ID_A > actual &&
    test_cmp expected actual
  ...
  test_expect_success "'ipfs pin remote rm' an existing pin by name" '
    ipfs pin remote rm --enc=json --name=name_b
  '

  test_expect_success "'ipfs pin remote rm' an existing pin by status" '
    ipfs pin remote rm --enc=json --status=pinned
  '
