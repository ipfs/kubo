...
test_repo_cmd() {
	...
	test_expect_success "'ipfs pin ls -type=direct' is correct" '
		...
		DPHASH=QmUNLLsPACCz1vLxQVkXqqLX5R1X345qqfHbsf67hvA3Nn &&
		echo "$DPHASH" >>directpinexpected &&
	...
	test_expect_success "'ipfs pin rm' succeeds" '
		ipfs pin rm "$DPHASH" &&
		ipfs pin rm "$DIRECTPIN" &&
		ipfs pin ls >rm_actual &&
		test_cmp empty rm_actual &&
		ipfs pin rm -r "$MBLOCKHASH" &&
	...
	test_expect_success "'ipfs repo gc' looks good" '
		echo "removed $DIRECTPIN" >gc_out_expected &&
		echo "removed $DPHASH" >>gc_out_expected &&
		echo "removed $HASH_WELCOME_DOCS" >>gc_out_expected &&
		echo "removed $MBLOCKHASH" >>gc_out_expected &&
