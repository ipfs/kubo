...
// TestAutoConfigExtensibility_NewSystem verifies that the AutoConfig system can be extended
// with new routing systems beyond the default AminoDHT and IPNI.
//
// The test verifies that:
// 1. New systems can be added via AutoConfig's SystemRegistry
...
func TestAutoConfigExtensibility_NewSystem(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping test in short mode")
	}

	// Setup mock autoconfig server with NewSystem
	var mockServer *httptest.Server
	mockServer = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Create autoconfig.json with NewSystem
		...
		_ = json.NewEncoder(w).Encode(autoconfigData)
	}))
	...
	// Update the autoconfig to point to the correct NewSystem endpoint
	mockServer.Close()
	mockServer = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		autoconfigData := map[string]interface{}{
			"AutoConfigVersion": 2025072901,
	...
	// Create Kubo node with autoconfig pointing to mock server
	h := harness.NewT(t)
	node := h.NewNode().Init()

	// Update config to use mock autoconfig server
	node.UpdateConfig(func(cfg *config.Config) {
		cfg.AutoConfig.URL = mockServer.URL
		cfg.AutoConfig.Enabled = config.True
		cfg.AutoConfig.RefreshInterval = config.NewOptionalDuration(1 * time.Second)
