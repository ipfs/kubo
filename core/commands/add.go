...
`,
	...
	Options: []cmds.Option{
		...
		cmds.BoolOption(rawLeavesOptionName, "Use raw blocks for leaf nodes. Implies CIDv1 when enabled. (experimental)"),
		cmds.BoolOption(noCopyOptionName, "Add the file using filestore. Implies raw-leaves. (experimental)"),
		cmds.BoolOption(fstoreCacheOptionName, "Check the filestore for pre-existing blocks. (experimental)"),
		cmds.IntOption(cidVersionOptionName, "CID version. Defaults to 0 unless an option that depends on CIDv1 (hash, raw-leaves) is passed."),
