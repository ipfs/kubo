...
`,
	...
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		...
		providerSystem, ok := nd.Provider.(provider.System)
		if !ok {
			return errors.New("configured provider system doesn't allow manual reprovides")
		}
		...
		err = providerSystem.Reprovide(req.Context)
		if err != nil {
			return err
		}
