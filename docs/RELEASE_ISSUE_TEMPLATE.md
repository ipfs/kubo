...
- [ ] Verify you have all the access and tools you need to perform the release
...
- [ ] `kuboreleaser release --version vX.Y.Z(-RCN) notify-bifrost --date YYYY-MM-DD`
  - [ ] Open an issue against [bifrost-infra](https://github.com/protocol/bifrost-infra) ahead of the release ([example](https://github.com/protocol/bifrost-infra/issues/2221)).
    - **Idealy, do this multiple days in advance of the RC** to give Bifrost the heads up that asks will be coming their way.
...
- [ ] `kuboreleaser release --version vX.Y.Z(-RCN) tag`
  - [ ] Create a signed tag for the release candidate.
    ...
    - ⚠️ Tag HEAD `release-vX.Y.Z`/`release` commit with `vX.Y.Z(-RCN)` (`git tag -s vX.Y.Z(-RCN) -m 'Release X.Y.Z(-RCN)'`)
