...
> The `flat` strategy can be used to lower memory requirements, but only if memory utilization is too high, prioritization of pins is not necessary, and it is acceptable to announce every block cached in the local repository.
