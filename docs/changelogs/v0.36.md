...
- [🔦 Highlights](#-highlights)
  ...
  - [Support `--flush` in `files rm` command](#support---flush-in-files-rm-command)
...
#### Support `--flush` in `files rm` command

The `ipfs files rm` command ignored `--flush=false` option. It is now respected, however should be used with caution: see WARNING in `ipfs files --help`.
