...
- New `--dereference-symlinks` flag for `ipfs add` resolves all symlinks to their target content, both CLI arguments and symlinks inside directories. This is a superset of the deprecated `--dereference-args` flag which only resolved CLI argument symlinks.
