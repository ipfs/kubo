...
Additionally, the MFS root directory itself now respects `Import.CidVersion` and `Import.HashFunction` at daemon startup. Before this fix, the root always used CIDv0/sha2-256 regardless of config.
