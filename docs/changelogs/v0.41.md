...
Until now, blocks added via the [filestore](https://github.com/ipfs/kubo/blob/master/docs/experimental-features.md#ipfs-filestore) or [urlstore](https://github.com/ipfs/kubo/blob/master/docs/experimental-features.md#ipfs-urlstore) (`ipfs add --nocopy`) ignored [`Provide.Strategy`](https://github.com/ipfs/kubo/blob/master/docs/config.md#providestrategy) and were eagerly announced at write time, so a node running a selective strategy would still flood the routing system with every chunk it ingested. The filestore is now gated on the strategy the same way the regular blockstore is.

The practical payoff is that filestore-backed content now plays well with the new [`--fast-provide-root` / `--fast-provide-dag` flags](#-new---fast-provide-dag-flag-for-fine-tuned-provide-control). Users on selective strategies (`pinned`, `pinned+mfs`, `pinned+mfs+entities`, ...) get the same fine-grained control over add-time provide traffic that regular `ipfs add` got in this release: only the root is announced via [`Import.FastProvideRoot`](https://github.com/ipfs/kubo/blob/master/docs/config.md#importfastprovideroot) by default, and setting [`Import.FastProvideDAG=true`](https://github.com/ipfs/kubo/blob/master/docs/config.md#importfastprovidedag) (or passing `--fast-provide-dag=true` per command) walks the full DAG right after add and honors the active strategy. `Provide.Strategy=all` (the default) is unaffected.
