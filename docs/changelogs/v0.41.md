...
- [🔦 Highlights](#-highlights)
  ...
  - [`ipfs object patch add-link` validates UnixFS node types](#ipfs-object-patch-add-link-validates-unixfs-node-types)
...
#### `ipfs object patch add-link` validates UnixFS node types

The deprecated `ipfs object patch add-link` command now rejects adding named
links to non-directory UnixFS nodes (e.g., files, symlinks). Previously, this
would silently create invalid UnixFS DAGs where file content was lost when
reading back. Only `Directory` and `HAMTShard` nodes support named links per the
[UnixFS specification](https://specs.ipfs.tech/unixfs/#pbnode-links-name).

A `--allow-non-unixfs` flag is available to bypass the validation when
operating on raw `dag-pb` nodes outside of UnixFS semantics.
