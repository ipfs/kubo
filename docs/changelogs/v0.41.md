...
- **UnixFS mode and mtime visible in stat.** All three mounts (`/ipfs`, `/ipns`, `/mfs`) now show POSIX mode and mtime from [UnixFS](https://specs.ipfs.tech/unixfs/) metadata when present. Most IPFS data does not include this optional metadata; when absent, sensible defaults are used.
- **Opt-in `Mounts.StoreMtime` and `Mounts.StoreMode` config flags.** Writable mounts (`/ipns`, `/mfs`) can now persist mtime on file creation/write and POSIX mode on `chmod`, storing them in UnixFS metadata. Both are off by default because they change the resulting CID. See [`Mounts.StoreMtime`](https://github.com/ipfs/kubo/blob/master/docs/config.md#mountsstoremtime) and [`Mounts.StoreMode`](https://github.com/ipfs/kubo/blob/master/docs/config.md#mountsstoremode).
- **Concurrent IPNS file operations no longer race.** The `/ipns` file handle now serializes Read, Write, Flush, and Release with a mutex, matching the `/mfs` mount.
- **IPNS directory operations flush immediately.** Remove and Rename on `/ipns` now flush changes to the MFS root, preventing data loss on daemon restart.
- **New files inherit CID builder from parent.** Files created via the `/ipns` mount now inherit the parent directory's CID builder, preventing CIDv0 nodes from appearing inside a CIDv1 tree.
- **Aligned default file permissions.** Both `/ipns` and `/mfs` now use standard POSIX defaults (files: `0644`, directories: `0755`), matching `umask 022`.
- **`ipfs.cid` xattr on all mounts.** All three mounts (`/ipfs`, `/ipns`, `/mfs`) now expose the node's CID via the `ipfs.cid` extended attribute on both files and directories. The old `ipfs_cid` name is still accepted on `/mfs` for backward compatibility but will be removed in a future release.
