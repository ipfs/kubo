...
- [🔦 Highlights](#-highlights)
  ...
  - [📌 Fast-provide root CID on `pin add` and `pin update`](#-fast-provide-root-cid-on-pin-add-and-pin-update)
...
#### 📌 Fast-provide root CID on `pin add` and `pin update`

`ipfs pin add` and `ipfs pin update` now immediately announce the root CID to the DHT after pinning, matching the existing behavior of `ipfs add` and `ipfs dag import`. Previously, pinned content was not discoverable until the next reprovide cycle (up to 22h). Controlled by `Import.FastProvideRoot` (default: true) and `Import.FastProvideWait` (default: false) config options.

#### 🛡️ Hardened `Provide.Strategy` parsing

`Provide.Strategy` now rejects unknown tokens at startup (e.g. typos like `"uniuqe"` instead of `"unique"`), malformed delimiters (`"pinned+"`), and invalid combinations (`"all+pinned"`). Previously, unknown tokens were silently ignored.
