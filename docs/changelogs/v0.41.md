...
- [🔦 Highlights](#-highlights)
  ...
  - [🔧 Smarter HTTP routing provider addresses](#-smarter-http-routing-provider-addresses)
...
#### 🔧 Smarter HTTP routing provider addresses

Provider addresses sent to HTTP routers are now resolved at provide-time instead of daemon startup. This mainly helps users running custom routing stacks (`Routing.Type=custom`) that depend on [IPIP-526](https://github.com/ipfs/specs/pull/526) and were getting unresolved `0.0.0.0` addresses in provider records instead of actual interface addresses. When available, confirmed reachable addresses from AutoNAT V2 are preferred. See [#11213](https://github.com/ipfs/kubo/issues/11213).
