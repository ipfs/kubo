...
- **No more deadlocks on save.** Editors that call `fsync` after writing would hang indefinitely on `/mfs` mounts. `Fsync` is now a no-op; data is flushed when the file descriptor is closed.
