...
- [🔦 Highlights](#-highlights)
  ...
  - [MFS DAG now included in reprovides](#mfs-dag-now-included-in-reprovides)
...
#### MFS DAG now included in reprovides

MFS (Mutable File System) is a Kubo feature consisting of a UnixFS filesystem managed via `ipfs files`. It allows to perform usual file operations like `cp` and `mv` in a folder-tree structure, transparently updating a merkledag an a "root cid" that references the current state of MFS. MFS is used by IPFS Desktop, which places uploaded files directly on it.

MFS CIDs, like CID in the pinset, are not GC'ed. However, until now, MFS CIDs, unlike those in the pinset, were not announced to the network (we call the process "(re)providing"). This has now changed and MFS CIDs will be provided by default just like all pinned CIDs are.

Additionally, a new `mfs` providing strategy has been added, which provides only MFS cids.

The `roots` providing strategy now provides pinned roots and the MFS root.
