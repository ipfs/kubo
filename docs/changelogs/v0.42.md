...
- [🔦 Highlights](#-highlights)
  ...
  - [🐛 Bounded graceful shutdown and other timeouts](#-bounded-graceful-shutdown-and-other-timeouts)
...
Daemon shutdown is also cleaner under these strategies: the pinner now cancels and drains in-flight work before the datastore closes, so the transient `pebble: closed` panic trace on exit is gone.

#### 🐛 Bounded graceful shutdown and other timeouts

Graceful shutdown could hang forever if any subsystem's `OnStop` hook never returned, leaving the daemon in a "half-shutdown" state: internal subsystems closed, RPC API still answering, invisible to container healthchecks.

The new `ipfs diag healthy` subcommand reports daemon liveness. It exits non-zero when shutdown has been initiated (even if the RPC API still answers) or when the daemon cannot resolve a built-in CID. The kubo Docker image's `HEALTHCHECK` now uses this command, so orchestrators detect and recycle half-shutdown daemons within seconds.

The new [`Internal.ShutdownTimeout`](https://github.com/ipfs/kubo/blob/master/docs/config.md#internalshutdowntimeout) is an extra knob for operators to cap how long graceful shutdown is allowed to take, mainly useful when running without an orchestrator-driven healthcheck. On expiry, the daemon logs which subsystem failed and exits with status `1`.

The DHT provider gained two related bounds. `ipfs provide stat` now honors client cancellation and deadlines instead of blocking indefinitely behind a slow keystore lookup (previously seen at >1h in production). Each `ADD_PROVIDER` RPC is now capped by [`Provide.DHT.SendProviderRecordTimeout`](https://github.com/ipfs/kubo/blob/master/docs/config.md#providedhtsendproviderrecordtimeout), so a peer that accepts the libp2p stream but never reads the request can no longer pin a provide worker and stall reprovide cycles.
