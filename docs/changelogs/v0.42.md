...
You can now push CIDs to the routing system on demand with `ipfs provide once <cid>...`, on top of the regular reprovide schedule. CIDs streamed through stdin flow into the command as they arrive, so you can pipe arbitrarily large lists without holding them in memory:
