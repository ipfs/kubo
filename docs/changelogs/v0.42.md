...
The one-time migration that runs when starting Kubo against a repo from `go-ipfs` or a Kubo older than v0.27 is now resilient to slow or blocked public gateways. Previously a single hard-coded gateway with no HTTP timeouts could hang the daemon indefinitely; now several gateways are tried with timeouts and a hard failure cap. Override via [`Migration.DownloadSources`](https://github.com/ipfs/kubo/blob/master/docs/config.md#migrationdownloadsources) if needed.
