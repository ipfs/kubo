...
Ad-hoc per-CID providing has moved into the `ipfs provide` namespace as `ipfs provide once <cid>...`. The command submits provider records through the provide system right away, on top of the regular reprovide schedule. See `ipfs provide once --help` for details.
