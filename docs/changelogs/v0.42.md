...
You can now push CIDs to the routing system on demand with `ipfs provide once <cid>...`, on top of the regular reprovide schedule. CIDs streamed through stdin flow into the command as they arrive, and a bloom-based dedup keeps daemon memory bounded even when piping large lists:
