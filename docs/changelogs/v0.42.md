...
- [🔦 Highlights](#-highlights)
  - [🎯 New `ipfs provide once` for ad-hoc announcements](#-new-ipfs-provide-once-for-ad-hoc-announcements)
...
#### 🎯 New `ipfs provide once` for ad-hoc announcements

Ad-hoc per-CID providing has moved into the `ipfs provide` namespace as `ipfs provide once <cid>...`. The command submits provider records through the provider system right away, on top of the regular reprovide schedule. See `ipfs provide once --help` for details.

`ipfs routing provide` is now deprecated. It keeps its existing behavior so existing scripts continue to work, but will be removed in a future release.
