...
- When ipfs is initialized, the datastore metrics wrapper is not configured by default as it previously was. This means metrics prefixed by `<dsname>_datastore` are no longer exposed. To enable datastore metrics during initialization, specify the appropriate `--profile` option. In the case of the `flatfs-measure` profile, `flatfs_datastore_...` and `leveldb_datastore_...` metrics are exposed. For a list of these see [`prometheus_metrics_added_by_measure_profile`](https://github.com/ipfs/kubo/blob/master/test/sharness/t0119-prometheus-data/prometheus_metrics_added_by_measure_profile).
  - New profile options: `flatfs-measure`, `pebbleds-measure`, `badgerds-measure`
