...
Kubo keeps blocks in flatfs and everything else (pins, MFS root, IPNS and DHT provider records) in leveldb, which is slow to reclaim space after bulk deletes: on nodes that churn that metadata all day, `datastore/` grows far past the live data. `ipfs init --profile=flatfs-pebbleds` keeps flatfs for blocks and puts the rest in pebble, which compacts deleted keys promptly. New repos only, experimental, and pebble has less production mileage in Kubo than leveldb, so report problems in [kubo issues](https://github.com/ipfs/kubo/issues). Details in the [profile docs](https://github.com/ipfs/kubo/blob/master/docs/config.md#flatfs-pebbleds-profile) ([#11423](https://github.com/ipfs/kubo/issues/11423)).

The default layout is unchanged; its profile is now named `flatfs-levelds`, with `flatfs` and `flatfs-measure` kept as aliases, and the [datastore docs](https://github.com/ipfs/kubo/blob/master/docs/datastores.md#flatfs) say which data lives in which store and why flatfs holds only blocks.

#### 🗂️ Setting the flatfs shard depth is less error-prone

The flatfs shard depth (`shardFunc`) can only be set when a repo is created, by passing a config file to `ipfs init`, and that route had traps. Fixed:
