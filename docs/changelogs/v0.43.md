...
- [🔦 Highlights](#-highlights)
  ...
  - [🔐 secp256k1 keys can be generated and exported again](#-secp256k1-keys-can-be-generated-and-exported-again)
...
#### 🔐 secp256k1 keys can be generated and exported again

`ipfs key gen --type=secp256k1` is back. It was removed in Kubo 0.21 because freshly generated keys could not be exported: the PEM PKCS #8 support relied on Go's `crypto/x509`, which does not know the secp256k1 curve ([#9912](https://github.com/ipfs/kubo/issues/9912)).

`ipfs key export --format=pem-pkcs8-cleartext` and `ipfs key import` now handle secp256k1 like the other key types, so keys move between Kubo and OpenSSL in both directions:
...
`ipfs key rotate --type=secp256k1` works as well.
