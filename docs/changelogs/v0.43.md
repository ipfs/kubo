...
#### 🧪 Experimental opt-in `flatfs-pebbleds` profile for heavy pin, IPNS, and DHT churn

Kubo keeps blocks in flatfs and everything else, such as pins, the MFS root, IPNS records, and DHT provider records, in leveldb. Nodes that churn that metadata all day (pinning services adding and removing pins, busy IPNS publishers, DHT servers with large provider tables) hit leveldb's weak spot: it is slow to reclaim space after bulk deletes, so `datastore/` grows far past the live data. The new `flatfs-pebbleds` profile keeps flatfs for blocks and swaps leveldb for pebble, which compacts deleted keys promptly. It is experimental and opt-in, for new repos only: `ipfs init --profile=flatfs-pebbleds`. Existing repos are not migrated. `flatfs-pebbleds-measure` adds per-datastore metrics, with overhead, for debugging and right-sizing. Pebble has less production mileage in Kubo than leveldb, so report problems in [kubo issues](https://github.com/ipfs/kubo/issues). Details in the [`flatfs-pebbleds` profile](https://github.com/ipfs/kubo/blob/master/docs/config.md#flatfs-pebbleds-profile) docs ([#11423](https://github.com/ipfs/kubo/issues/11423)).

The default layout is unchanged. Its profile is now named `flatfs-levelds` so the name says what goes where; `flatfs` and `flatfs-measure` keep working as aliases. The [datastore docs](https://github.com/ipfs/kubo/blob/master/docs/datastores.md#flatfs) explain which data lives in which store and why flatfs holds only blocks, and `ipfs init --help` summarizes the profiles.

#### 🗂️ Setting the flatfs shard depth is less error-prone

The flatfs shard depth (`shardFunc`) can only be set when a repo is created, by passing a config file to `ipfs init`, and that route had traps: `ipfs config show` output was accepted even though it lacks the private key and produced a repo that crashed on start, a typo in `shardFunc` left a half-made repo that the next `ipfs init` refused to touch, and a hand-edited `shardFunc` on an existing repo failed with an error that swapped the two values it printed. Fixed:

- `ipfs init <file>` and `ipfs daemon --init --init-config` refuse a config without a private key and say to use a copy of a repo's `config` file instead.
- `ipfs init` checks `Datastore.Spec` before writing anything, so a bad `shardFunc` fails cleanly.
- `ipfs config profile apply` refuses a profile that would change the datastore layout of an existing repo, which is fixed at init, instead of writing a config the repo can no longer open.
- The error for a `Datastore.Spec` that no longer matches the repo's `datastore_spec` file labels the two values correctly and says what to do.

[`docs/datastores.md`](https://github.com/ipfs/kubo/blob/master/docs/datastores.md#choosing-a-shardfunc-for-large-blockstores) now has the step list for setting `next-to-last/3` on a new repo, when it is worth it (roughly past 10M blocks, and only after measuring on your own disk with your own blocks), what it costs, and how to move an existing repo to it. Defaults are unchanged.
