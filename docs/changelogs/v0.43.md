...
#### 🔒 Hardened CID profiles and a new low-level knob

The `unixfs-v0-2015` and `unixfs-v1-2025` profiles now pin the `PBNode` field order (`links-first`) explicitly, and regression tests lock in the exact bytes and CIDs they produce, so future changes cannot alter them silently. Reading blocks in either field order is also formalized and tested: [gateway-conformance v0.14.1](https://github.com/ipfs/gateway-conformance/releases/tag/v0.14.1) checks that gateways serve UnixFS directories and HAMT shards in both encodings ([IPIP-550](https://github.com/ipfs/specs/pull/550)).

As a side effect, a new low-level [`Import.UnixFSPBNodeFieldOrder`](https://github.com/ipfs/kubo/blob/master/docs/config.md#importunixfspbnodefieldorder) option lets writers opt into `Data`-first output, which lets streaming readers interpret a sharded (HAMT) directory block before buffering all its links. Nothing changes by default: no profile enables the new order, and Kubo keeps producing the same bytes and CIDs while accepting both orders when reading, as it always has. Opting in changes the CIDs of new directories, HAMT shards, and files larger than one chunk, and re-encodes data already in MFS as `ipfs files` commands read it, so enable it only when consumers of your CIDs expect the new order.
