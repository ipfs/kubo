...
`ipfs init` now creates repositories that default to CIDv1 (base32, starting with `bafy` or `bafk`) instead of CIDv0 (base58 `Qm...`). Init applies the [`unixfs-v1-2025`](https://github.com/ipfs/kubo/blob/master/docs/config.md#unixfs-v1-2025-profile) import profile, so `ipfs add`, `ipfs files`, and the initial MFS root produce CIDv1 with raw leaves, 1 MiB chunks, and the other [IPIP-499](https://specs.ipfs.tech/ipips/ipip-0499/) settings.
...
Existing repositories are left untouched: there is no migration and no repo-version bump, so a node that upgrades keeps producing the same CIDs as before, and only a fresh `ipfs init` gets the new default. A repo whose UnixFS import settings are still implicit now prints a one-time notice at daemon startup suggesting you pin a profile so its CIDs stay stable across releases: [`unixfs-v1-2025`](https://github.com/ipfs/kubo/blob/master/docs/config.md#unixfs-v1-2025-profile) for the modern defaults, or [`unixfs-v0-2015`](https://github.com/ipfs/kubo/blob/master/docs/config.md#unixfs-v0-2015-profile) if you need to keep reproducing your existing CIDs. If you need to init with the legacy CIDv0 profile, use `ipfs init --profile unixfs-v0-2015`.

CIDv0 and raw leaves do not mix: raw leaves are a CIDv1-only encoding, so asking for both used to silently emit an unexpected CID. Kubo now catches this mismatch instead, on every path that ingests data (`ipfs add` and other CLI/RPC commands, and the `Import.*` config):

- `ipfs add --cid-version=0` forces dag-pb leaves, so a small file comes out as a `Qm...` CID rather than a raw `bafk...` leaf.
- Passing `--cid-version=0` together with `--raw-leaves` is a contradiction and now fails with a clear error instead of quietly producing a CIDv1 leaf.
- A repo whose config sets `Import.CidVersion=0` together with `Import.UnixFSRawLeaves=true` is rejected at daemon startup with the same error, so the mistake is caught before it can produce surprising CIDs.

Programs using the `github.com/ipfs/kubo/client/rpc` Go library get the same behavior: `Unixfs().Add` now follows the daemon's configured CID version when the caller doesn't set one, instead of always requesting CIDv0, so against a CIDv1 daemon it returns `bafy...`/`bafk...` CIDs like `ipfs add`. Its `MaxFileLinks`, `MaxDirectoryLinks`, and `MaxHAMTFanout` add options are now forwarded to the daemon too; before, they were silently ignored.
...
- update `go-libp2p` to a pre-release pinned at [ec408fcc](https://github.com/libp2p/go-libp2p/commit/ec408fcc60c9), ahead of v0.48.0 (no tagged release yet)
