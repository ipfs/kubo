...
- update `go-libp2p-kad-dht` to a pre-release pinned at [2cba273e](https://github.com/libp2p/go-libp2p-kad-dht/commit/2cba273e45d8eb02e60a8bb3b668d523b800c32d), ahead of v0.42.0 (incl. [v0.42.0](https://github.com/libp2p/go-libp2p-kad-dht/releases/tag/v0.42.0), [v0.41.0](https://github.com/libp2p/go-libp2p-kad-dht/releases/tag/v0.41.0) and the [local record validation fix](https://github.com/libp2p/go-libp2p-kad-dht/pull/1285))
  - DHT value records (`pk`, `ipns`) are now stored under namespaced datastore keys, and Kubo scopes them under the dedicated `/dht` prefix of the repo datastore (`/dht/pk/...`, `/dht/ipns/...`), keeping DHT-cached records separate from data owned by other subsystems. Provider records keep their existing dedicated `/providers/...` prefix and survive the upgrade. Records stored in the old root-level format are unreachable by the new code; Kubo purges them from the repo datastore once, in the background, on the first daemon run after the upgrade.
  - The offline router (used by offline nodes, `--offline` commands, and the `Gateway.NoFetch` gateway) now stores value records in the same datastore layout as the DHT (via [boxo#1189](https://github.com/ipfs/boxo/pull/1189)), so names published online stay resolvable offline and names published offline are served once the node comes online. Previously this only worked because boxo's offline router happened to share the pre-v0.42.0 DHT key format.
- update `boxo` to a pre-release pinned at [a47e98c9](https://github.com/ipfs/boxo/commit/a47e98c95ee0b954a0a4ffa1041e141c657da3e2), including the offline router change above (also pulls in `go-doh-resolver` [v0.6.0](https://github.com/libp2p/go-doh-resolver/releases/tag/v0.6.0) and `go-multiaddr-dns` [v0.6.0](https://github.com/multiformats/go-multiaddr-dns/releases/tag/v0.6.0))
