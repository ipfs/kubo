...
Streaming a sharded (HAMT) directory was more expensive than it needed to be: the parameters required to interpret its entries live in the `Data` field, which the legacy encoding places after all the links, so a reader looking for one entry had to read every link first. The new opt-in [`unixfs-v1-2026` profile](https://github.com/ipfs/kubo/blob/master/docs/config.md#unixfs-v1-2026-profile) fixes this for newly added data by writing the `Data` field first ([`Import.UnixFSPBNodeFieldOrder`](https://github.com/ipfs/kubo/blob/master/docs/config.md#importunixfspbnodefieldorder)), so streaming readers can stop as soon as they find the entry they want ([IPIP-550](https://github.com/ipfs/specs/pull/550)).

Nothing changes unless you opt in: the default behavior and the preexisting `unixfs-v0-2015` and `unixfs-v1-2025` profiles keep producing the same bytes and CIDs as before, and Kubo keeps accepting both encodings when reading, as it always has. Producing the new order changes CIDs compared to `unixfs-v1-2025`, so apply it (`ipfs config profile apply unixfs-v1-2026`) only when consumers of your CIDs expect this profile.
