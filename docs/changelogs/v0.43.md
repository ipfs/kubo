...
Running `ipfs repo gc` at the same time as `ipfs files` writes (`write`, `cp`, `mkdir`, `rm`, `flush`) could get the whole MFS (Mutable File System) stuck: every later `ipfs files` command would hang, and the daemon had to be force-killed to recover. It mostly hit busy nodes that write to MFS from several processes at once and run GC often.

The cause was garbage collection deleting blocks that an in-progress write had just added but had not yet linked into the saved MFS root, leaving the tree pointing at data that no longer existed. Kubo now takes the same lock `ipfs add` uses around each `ipfs files` change and reads the MFS root while that lock is held, so GC can no longer collect data a write still needs. As with `ipfs add`, a garbage collection run and in-flight `ipfs files` writes now briefly wait for each other. Nodes that avoided GC or scheduled restarts to work around this no longer need those workarounds.
