...
#### 🌊 Cheaper streaming reads of big directories (opt-in `unixfs-v1-2026`)

Finding one entry in a big sharded (HAMT) directory starts with two parameters, the hash function and the fanout, that tell the reader which shard holds the name it wants. The legacy encoding stores them in the `Data` field after all the links, so a streaming reader must buffer every link in a block before it can interpret a single one. The opt-in [`unixfs-v1-2026` profile](https://github.com/ipfs/kubo/blob/master/docs/config.md#unixfs-v1-2026-profile) writes the `Data` field first ([`Import.UnixFSPBNodeFieldOrder`](https://github.com/ipfs/kubo/blob/master/docs/config.md#importunixfspbnodefieldorder)), so the reader knows the layout up front and can stop at the entry it wants ([IPIP-550](https://github.com/ipfs/specs/pull/550)).

Nothing changes unless you opt in: the defaults and the preexisting `unixfs-v0-2015` and `unixfs-v1-2025` profiles keep producing the same bytes and CIDs, and Kubo keeps accepting both orders when reading, as it always has. Opting in (`ipfs config profile apply unixfs-v1-2026`) changes the CIDs of new directories, HAMT shards, and files larger than one chunk, so do it only when consumers of your CIDs expect this profile. It also upgrades data already in MFS: the first `ipfs files` command that reads a directory, even a plain `ipfs files ls` or `ipfs files stat`, re-encodes it in the new order and gives it a new CID (a sharded directory's root first, its child shards as they are read). Content is unchanged and both orders stay readable, but expect MFS CIDs to change on first use after the switch.
