...
- [🔦 Highlights](#-highlights)
  - [AutoConfig: Complete control over network defaults](#autoconfig-complete-control-over-network-defaults)
...
#### AutoConfig: Complete control over network defaults
...
Your config file now explicitly shows `["auto"]` placeholders that resolve to curated, up-to-date network defaults. You can inspect these settings anytime, replace them with your own services, or disable the AutoConfig feature entirely.
...
- Switch between automatic and manual modes with `--profile=autoconfig-on|autoconfig-off`
- Control the system with `AutoConfig.Enabled` and customize the source with `AutoConfig.URL`

**Take control of your setup:**
```bash
# Switch to fully automatic network configuration
ipfs config profiles apply autoconfig-on

# Switch to manual control - explicitly set all AutoConfig fields
ipfs config profiles apply autoconfig-off
...
**Use your own AutoConfig manifest:**

Organizations can create custom AutoConfig manifests tailored to their swarm or client needs - private networks, enterprise deployments, or specialized applications can define their own bootstrap peers, DNS resolvers, and routing endpoints.

Host your autoconfig.json anywhere and configure with `AutoConfig.URL`. See AutoConfig format and guidance at https://config.ipfs-mainnet.org/
...
Configure HTTP endpoints for IPNS publishing using [`Ipns.DelegatedPublishers`](https://github.com/ipfs/kubo/blob/master/docs/config.md#ipnsdelegatedpublishers). Like other AutoConfig fields, it supports the special `"auto"` value to use curated network defaults, or specify your own HTTP endpoints for full control over where your IPNS records are published.
