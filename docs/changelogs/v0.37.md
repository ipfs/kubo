...
- **`Gateway.RetrievalTimeout`**: Enforces maximum duration for content retrieval (default: 30s). Returns 504 Gateway Timeout when the gateway cannot retrieve content within this period - both for initial retrieval (time to first byte) and between subsequent writes.
- **`Gateway.MaxConcurrentRequests`**: Limits concurrent HTTP requests (default: 4096). Returns 429 Too Many Requests when exceeded. This provides a tunable limiter to protect nodes from traffic spikes, high load and resource exhaustion when running behind a reverse proxy without proper rate-limiting. The default acts as a failsafe of last resort for most deployments.

New Prometheus metrics for monitoring:
- `ipfs_http_gw_concurrent_requests`: Current number of requests being processed
- `ipfs_http_gw_responses_total`: Total HTTP responses by status code
