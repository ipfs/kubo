...
- `ipfs log level foo` - Shows log level for a specific  facility only
- HTTP API: `POST /api/v0/log/get-level?arg=<subsystem>`

This modifies the existing `ipfs log level` command, which was previously only used for setting log levels. The updated command provids full visibility into your current logging configuration.
