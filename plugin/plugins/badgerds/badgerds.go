...
func (c *datastoreConfig) Create(path string) (repo.Datastore, error) {
	fmt.Println("⚠️ badgerds is based on badger 1.x, which has known bugs and is no longer supported by the upstream team. Please switch to a newer datastore such as pebbleds or flatfs.")
	p := c.path
	if !filepath.IsAbs(p) {
