...
func getBalancedDag(t testing.TB, size int64, blksize int64) (*dag.Node, dag.DAGService) {
	...
	nd, err := BuildDagFromReader(ds, chunk.NewSizeSplitter(r, blksize), 0)
	if err != nil {
		t.Fatal(err)
	}
	return nd, ds
...
func getTrickleDag(t testing.TB, size int64, blksize int64) (*dag.Node, dag.DAGService) {
	ds := mdtest.Mock()
	r := io.LimitReader(u.NewTimeSeededRand(), size)
	nd, err := BuildTrickleDagFromReader(ds, chunk.NewSizeSplitter(r, blksize), 0)
	if err != nil {
		t.Fatal(err)
	}
	return nd, ds
...
func TestBalancedDag(t *testing.T) {
	...
	nd, err := BuildDagFromReader(ds, chunk.DefaultSplitter(r), 0)
	if err != nil {
		t.Fatal(err)
	}
