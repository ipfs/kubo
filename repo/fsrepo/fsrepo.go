...
type FSRepo struct {
	...
	userRessourceOverrides rcmgr.PartialLimitConfig
	ds                     repo.Datastore
...
func open(repoPath string, userConfigFilePath string) (repo.Repo, error) {
	...
	if err := r.openUserRessourceOverrides(); err != nil {
		return nil, err
	}
...
func (r *FSRepo) UserRessourceOverrides() (rcmgr.PartialLimitConfig, error) {
	// It is not necessary to hold the package lock since the repo is in an
	// opened state. The package lock is _not_ meant to ensure that the repo is
	// thread-safe. The package lock is only meant to guard against removal and
	// coordinate the lockfile. However, we provide thread-safety to keep
	...
	return r.userRessourceOverrides, nil
}
